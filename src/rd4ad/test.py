...
from rd4ad.dataset import MVTecDataset, get_data_transforms, load_data
...
def evaluation(encoder, bn, decoder, test_dataloader, device, _class_=None):
    ...
    with torch.no_grad():
        for img, gt, label, _ in test_dataloader:
            ...
            if label.item() != 0:
                aupro_list.append(
                    compute_pro(
                        masks=gt.squeeze(0).cpu().numpy().astype(int),
                        amaps=anomaly_map[np.newaxis, :, :],
                    )
                )
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        dfs.append({"pro": mean(pros), "fpr": fpr, "threshold": th})
