...
_DATASETS = {
    ...
    "condition-ds": ["patchcore.datasets.condition_ds", "ConditionDataset"],
}
...
def run(methods, results_path, gpu, seed, save_segmentation_images):
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            paths = [
                tuple_[-2] for tuple_ in dataloaders["testing"].dataset.data_to_iterate
            ]
            dfs.append(
                pd.DataFrame({"path": paths, "score": scores}).assign(
    ...
    dst = os.path.join(results_path, "preds.csv")
    pd.concat(dfs).to_csv(dst)
