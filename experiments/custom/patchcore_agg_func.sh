...
"
CHECKPOINT=" \
    prefix-experiment ablation-patch-mean \
    checkpoint-evaluations \
    checkpoint-model \
    run \
"
python -m anomaly_detection $COMMON "--cfg=dict($CFG pixel_agg_func='mean', patchsize=1)" $CHECKPOINT
python -m anomaly_detection $COMMON "--cfg=dict($CFG pixel_agg_func='mean', patchsize=29)" $CHECKPOINT
python -m anomaly_detection $COMMON "--cfg=dict($CFG pixel_agg_func='max', patchsize=1)" $CHECKPOINT
python -m anomaly_detection $COMMON "--cfg=dict($CFG pixel_agg_func='max', patchsize=29)" $CHECKPOINT