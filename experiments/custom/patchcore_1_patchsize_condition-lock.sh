...
CFG="\
    ...
    feature_retention_pct=0.0036739305384667596, \
    imagesize=(3, 224, 224), \
    layers_to_extract_from='layer2', \
    n_samples=20, \
    pretrain_embed_dimension=352, \
    target_embed_dimension=352, \
"
    # patchsize=40, \
...
python -m anomaly_detection $DS_2 --model-name=$MODEL_RN --weights=$WEIGHT_RN_2_SP $TASK_PC "--cfg=dict($CFG pixel_agg_func='mean', patchsize=1)" prefix-experiment ablation-patch-mean-patchsize1_ $CHECKPOINT